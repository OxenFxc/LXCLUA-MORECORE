...
static const char *findvararg (CallInfo *ci, int n, StkId *pos) {
  if (clLvalue(s2v(ci->func.p))->p->is_vararg & PF_VAHID) {
...
** ====================================================
**
** Concept: A Lua closure = Proto *p (code) + UpVal *upvals[] (captured vars)
** Hotfix swaps p but keeps upvals[] from the old closure.
**
** This allows: debug.hotfix(oldFunc, newFunc)
** - oldFunc executes newFunc's bytecode
** - oldFunc still accesses its original upvalues
*/
...
LUA_API void luaB_hotfix (lua_State *L, int oldidx, int newidx) {
  ...
  if (!lua_isfunction(L, oldidx) || !lua_isfunction(L, newidx)) {
    luaG_runerror(L, "attempt to hotfix a non-function value");
  }
  
  int oldnup = 0;
  while (lua_getupvalue(L, oldidx, oldnup + 1) != NULL) {
    oldnup++;
    lua_pop(L, 1);
  }
  
  int newnup = 0;
  while (lua_getupvalue(L, newidx, newnup + 1) != NULL) {
    newnup++;
    lua_pop(L, 1);
  }
  
  if (oldnup != newnup) {
    luaG_runerror(L, "upvalue count mismatch: old=%d, new=%d", oldnup, newnup);
  }
  
  if (oldnup == 0) {
    luaG_runerror(L, "hotfix requires closures with upvalues");
  }
  
  int oldabs = lua_absindex(L, oldidx);
  int newabs = lua_absindex(L, newidx);
  
  lua_pushvalue(L, oldabs);
  lua_pushvalue(L, newabs);
