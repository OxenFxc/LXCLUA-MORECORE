-- lexer_cff_abyss_mode.lua
-- ABYSS MODE V2: THE PINNACLE OF STABLE CONTROL-FLOW FLATTENING
-- Combines all previous features, fixes AST parsing edge cases, and adds ultra-realistic bogus blocks.
...
-- Generates realistic looking bogus code using the virtual memory array
local function generate_realistic_bogus_code(var_map, next_v_idx)
    ...
    local fake_code = {
        ...
        "local _fake_str = " .. string.format("%q", "err_" .. math.random(100, 999)),
        "_G['type'](__M["..v_idx.."])",
        "for _fake_i = 1, " .. math.random(2, 5) .. " do __M[200] = __M[200] + 1 end"
    }
...
local function extract_params(func_node, var_map, next_v_idx)
    -- The parameters are strictly at the beginning of the function declaration.
    -- We only look at the tokens before the main body block starts.
    -- Usually a function AST node has elements: `function` `<name>` `(` `params` `)` `body` `end`
...
local function flatten_abyss(func_node)
            ...
            elseif r.type == "<name>" then
                ...
                elseif is_keyword(r.value) then
                    ...
                    elseif prev_val == "for" or prev_val == "local" then
                        -- DO NOT obscure the control variable in a for loop, or a local declaration inside an unflattened block
                        -- To prevent bodies from treating it as global, map it to a bypass
                        var_map[r.value] = r.value -- Bypass it
                        table.insert(out_stmt, r)
                    else
                        if var_map[r.value] == r.value then
                            -- Bypassed local loop variable
                            table.insert(out_stmt, r)
                        else
                            -- Not a mapped local, not a keyword, not a property. Hide it!
        ...
        if names then
            ...
            if #rest > 0 then
                ...
                for idx, n in ipairs(names) do
                    ...
                    if idx < #names then
                        table.insert(new_stmt, {token=0, type="<raw>", value=",", line=stmt[1].line})
                    end
                end
                -- We stripped local declarations, the assignment needs the '=' which is in `rest` if it exists.
                -- Wait, lexer.parse_local returns `rest` starting with `=`.
    ...
    -- Map function args into the virtual memory array
    local map_args = ""
    for arg_name, arg_idx in pairs(var_map) do
        if type(arg_idx) == "number" and arg_idx < next_v_idx then
            -- We assume args mapped at the beginning before locals are actual arguments.
            -- To be safe, we just assign the param variable "__Mx" into the array "__M[x]"
    ...
    for _, z in ipairs(z_ids) do
        ...
        for _, g in ipairs(g_ids) do
            ...
            for _, state in ipairs(stage_list) do
                ...
                -- For Bogus, we use a false opaque predicate
                if state.is_bogus then
                    ...
                    -- Jump out to a random real state? No, bogus states are unreachable.
                    -- But if it was reached, jump to garbage.
                    ...
                    if not is_unconditional_return then
                        local xor_z = math.random(10, 99)
                        local add_z = state.next_z - (z ~ xor_z)
                        local xor_g = math.random(10, 99)
                        local add_g = state.next_g - (g ~ xor_g)
                        local xor_s = math.random(10, 99)
...
local code = [[
local function abyss_v2_example(start_val)
...
abyss_v2_example(100)
]]

print("--- Original Code ---")
print(code)

print("--- Abyss V2 Mode CFF Obfuscated Code ---")
