...
/* 判断字符是否为扩展字符（组合字符） */
static int utf8_is_extended(unsigned ch) {
    /* 组合字符包括：标记、间距标记、连接符等 */
    /* 参考Unicode标准的Grapheme Cluster Boundary Rules */
    return find_in_range(compose_table, table_size(compose_table), ch);
}

/* 获取下一个字素簇的起始位置 */
static const char *utf8_next_grapheme(const char *s, const char *e) {
    if (s >= e) return s;
    
    /* 获取当前字符 */
    unsigned ch;
    const char *next = s + utf8_decode(s, e, &ch);
    
    /* 如果是基本字符，检查后面是否有扩展字符 */
    while (next < e) {
        unsigned next_ch;
        const char *next_next = next + utf8_decode(next, e, &next_ch);
        if (!utf8_is_extended(next_ch)) {
            break;
        }
        next = next_next;
    }
...
static int push_offset(lua_State *L, const char *s, const char *e, const char *cur, lua_Integer offset) {
    ...
    char_len = utf8_decode(cur, e, &ch);
    lua_pushinteger(L, cur-s+1);
    lua_pushinteger(L, ch);
    lua_pushinteger(L, cur-s+char_len);
    return 3;
}
...
/* 字素簇迭代器辅助函数 */
static int graphemes_aux(lua_State *L) {
    /* 使用Lua API获取字符串及其长度 */
    size_t len;
    const char *s = lua_tolstring(L, lua_upvalueindex(1), &len);
    const char *e = s + len;
    size_t current_pos = (size_t)lua_tointeger(L, lua_upvalueindex(2));
    ...
    const char *current = s + current_pos;
    
    /* 找到下一个字素簇的结束位置 */
    const char *next = utf8_next_grapheme(current, e);
    
    /* 计算字素簇的起始索引（从1开始） */
    int index = 1;
    const char *temp = s;
    while (temp < current) {
        temp = utf8_next_grapheme(temp, e);
        index++;
    }
    
    /* 推送结果：索引和字素簇 */
    lua_pushinteger(L, index);
    lua_pushlstring(L, current, next - current);
    
    /* 更新状态 */
    lua_pushinteger(L, next - s);
...
/* utf8.graphemes 主函数 */
static int Lutf8_graphemes(lua_State *L) {
    luaL_checkstring(L, 1);
    /* 复制字符串到栈顶 */
    lua_pushvalue(L, 1);
    /* 压入初始位置（0） */
    lua_pushinteger(L, 0);
    /* 创建闭包，包含2个上值：字符串和当前位置 */
    lua_pushcclosure(L, graphemes_aux, 2);
    return 1;
}
