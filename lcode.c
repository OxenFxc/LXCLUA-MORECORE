...
static void discharge2reg (FuncState *fs, expdesc *e, int reg) {
  ...
  switch (e->k) {
    ...
    case VCALL: {
      /* 函数调用结果：u.info 是指令pc，需要从中提取目标寄存器，然后移动 */
      int call_reg = GETARG_A(fs->f->code[e->u.info]);
      if (reg != call_reg)
        luaK_codeABC(fs, OP_MOVE, reg, call_reg, 0);
      break;
    }
    case VNONRELOC: {
      if (reg != e->u.info)
...
void luaK_storevar (FuncState *fs, expdesc *var, expdesc *ex) {
  switch (var->k) {
    case VLOCAL: {
      freeexp(fs, ex);
      luaK_exp2reg(fs, ex, var->u.var.ridx);  /* compute 'ex' into proper place */
      return;
    }
    case VUPVAL: {
...
/*
** jumponnilcond - 当表达式为nil时生成条件跳转指令
** 功能描述：生成OP_TESTNIL指令，用于空值合并运算符(??)的实现
** 参数说明：
**   fs - 函数状态
**   e - 表达式描述符
**   cond - 条件标志（1表示nil时跳转，0表示非nil时跳转）
** 返回值：跳转指令的位置
*/
static int jumponnilcond (FuncState *fs, expdesc *e, int cond) {
  discharge2anyreg(fs, e);
  freeexp(fs, e);
  return condjump(fs, OP_TESTNIL, NO_REG, e->u.info, 0, cond);
}


/*
** luaK_goifnil - 当表达式为nil时继续执行，否则跳转
** 功能描述：用于空值合并运算符(??)的代码生成
**           如果表达式为nil则继续执行（取右操作数），否则跳转（保留左操作数）
** 参数说明：
**   fs - 函数状态
**   e - 表达式描述符
*/
void luaK_goifnil (FuncState *fs, expdesc *e) {
  ...
  switch (e->k) {
    case VNIL: {
      pc = NO_JUMP;  /* 总是nil，不需要跳转，继续计算右操作数 */
      break;
    }
    case VK: case VKFLT: case VKINT: case VKSTR: case VTRUE: case VFALSE: {
      /* 常量值（非nil），总是跳转保留左操作数 */
      discharge2anyreg(fs, e);
      freeexp(fs, e);
      pc = luaK_jump(fs);
    ...
    default: {
      pc = jumponnilcond(fs, e, 0);  /* k=0: nil时跳过JMP继续执行，非nil时执行JMP跳转 */
      break;
    }
  }
  luaK_concat(fs, &e->t, pc);  /* insert new jump in 't' list */
...
void luaK_infix (FuncState *fs, BinOpr op, expdesc *v) {
  ...
  switch (op) {
    ...
    case OPR_NULLCOAL: {
      luaK_goifnil(fs, v);  /* 只有当 'v' 为 nil 时才继续执行 */
      break;
    }
    case OPR_CONCAT: {
...
void luaK_posfix (FuncState *fs, BinOpr opr,
  ...
  switch (opr) {
    ...
    case OPR_NULLCOAL: {
      /* 空值合并运算符: a ?? b
      ** 如果 a 不是 nil，则结果为 a；否则结果为 b
      ** 与 OPR_OR 类似，但只检测 nil，不检测 false
      */
      lua_assert(e1->f == NO_JUMP);  /* list closed by 'luaK_infix' */
      luaK_concat(fs, &e2->t, e1->t);
      *e1 = *e2;
    ...
    case OPR_SPACESHIP: {
      ...
      */
      ...
      freeexps(fs, e1, e2);
      e1->u.info = luaK_codeABC(fs, OP_SPACESHIP, 0, r1, r2);
      e1->k = VRELOC;
      luaK_fixline(fs, line);
      break;
    }
...
** 
** @param fs 函数状态
** @param e1 左侧参数表达式（管道输入）
** @param e2 右侧函数表达式（管道目标）
*/
...
/*
** 实现反向管道运算符：f <| x 等价于 f(x)
** 将右侧表达式作为参数传递给左侧函数
** 与正向管道语义相同，但语法顺序相反
** 
** @param fs 函数状态
** @param e1 左侧函数表达式（管道目标）
** @param e2 右侧参数表达式（管道输入）
*/
...
void luaK_safepipe (FuncState *fs, expdesc *e1, expdesc *e2) {
  int func_reg, arg_reg, result_reg;
  int jmp_skip;
  int e1_reg = -1;
   ...
   */
  luaK_codeABCk(fs, OP_TESTNIL, arg_reg, arg_reg, 0, 1);
  jmp_skip = luaK_jump(fs);  /* 是 nil 时执行此 JMP 跳过函数调用 */
  
  /* 步骤4：不是 nil，生成函数调用指令 */
  luaK_codeABC(fs, OP_CALL, func_reg, 2, 2);
  
  /* 步骤7：生成跳转到结束的指令（跳过 nil 处理） */
  int jmp_end = luaK_jump(fs);
  
  /* 步骤8：修补 nil 跳转目标，将结果设为 nil */
  luaK_patchtohere(fs, jmp_skip);
  luaK_codeABC(fs, OP_LOADNIL, result_reg, 0, 0);  /* R[result] = nil */
...
/*
** 实现带参数的管道调用：x |> f(a, _, b)
** 管道值插入到占位符位置
** 
** @param fs 函数状态
** @param pipe_arg 管道参数表达式
** @param func_reg 函数所在寄存器
** @param nargs 总参数数量（包含占位符）
** @param placeholder_pos 占位符位置（0-based，-1表示追加到末尾）
*/
void luaK_pipe_call (FuncState *fs, expdesc *pipe_arg, int func_reg, 
  ...
  if (placeholder_pos < 0) {
    /* 无占位符，追加到参数列表末尾 */
    luaK_exp2nextreg(fs, pipe_arg);
    nargs++;
  } else {
    /* 有占位符，需要将参数插入到指定位置 */
    /* 参数位置：func_reg + 1 + placeholder_pos */
    int target_reg = func_reg + 1 + placeholder_pos;
    luaK_exp2reg(fs, pipe_arg, target_reg);
  }
  
  /* 生成函数调用指令 */
  pipe_arg->u.info = luaK_codeABC(fs, OP_CALL, func_reg, nargs + 1, 2);
  pipe_arg->k = VCALL;
  pipe_arg->t = NO_JUMP;
  pipe_arg->f = NO_JUMP;
...
/*
** 生成三元条件表达式的代码：exp1 ? exp2 : exp3
** 等价逻辑：(exp1 and exp2) or exp3
** 通过条件跳转实现精确的语义控制
*/
void luaK_condexp (FuncState *fs, expdesc *v1, expdesc *v2, expdesc *v3) {
  int exit;  /* 跳出三元表达式的跳转点 */
  int jf;  /* 条件为假时的跳转点 */
  
  /* 确保条件表达式v1在寄存器中 */
  luaK_exp2anyreg(fs, v1);
  
  /* 生成条件跳转：如果v1为假，跳转到v3的计算 */
  ...
  /* 设置结果表达式：结果在freereg-1寄存器中 */
  v1->k = VNONRELOC;
  v1->u.info = fs->freereg - 1;
  v1->t = NO_JUMP;
  v1->f = NO_JUMP;
}
